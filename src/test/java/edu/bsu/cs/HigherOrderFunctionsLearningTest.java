...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(l -> l.charAt(0)).distinct().map(Object::toString).collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(l -> l.endsWith("0") || l.endsWith("2") || l.endsWith("4") || l.endsWith("6") || l.endsWith("8")).collect(Collectors.joining());
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(revision -> !revision.user.toLowerCase(Locale.ROOT).contains("bot")).count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = Collections.max(input.collect(Collectors.groupingBy(revision -> LocalDateTime.ofInstant(revision.timestamp, ZoneId.systemDefault()).getMonth(), Collectors.counting())).entrySet(), Map.Entry.comparingByValue()).getKey();
